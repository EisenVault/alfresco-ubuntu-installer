...
|-------------|-----|-------------|
| Alfresco Content App | http://localhost/ | admin / (see `alfresco.env`) |
| Alfresco Repository | http://localhost/alfresco/ | admin / (see `alfresco.env`) |
| Alfresco Share | http://localhost/share/ | admin / (see `alfresco.env`) |
| Solr Admin | http://localhost:8983/solr/ | (uses secret header) |
| ActiveMQ Console | http://localhost:8161/ | (see `alfresco.env`) |

> **Note**: Step 00 securely prompts for the Alfresco admin password and installation home. The default home is `/opt/eisenvault_installations/eisenvault-alfresco-26_2`; press Enter to accept it or enter another absolute path.
...
Generated by `00-generate-config.sh` with secure random service credentials. The script prompts for the Alfresco admin password rather than generating one:
...
# Admin User (only applied on fresh installation)
ALFRESCO_ADMIN_PASSWORD="<password-entered-during-step-00>"
ALFRESCO_ADMIN_PASSWORD_HASH="<MD4-hash-of-entered-password>"
...
The installer automatically generates a secure random password for the Alfresco admin user. This password is only applied during the initial repository bootstrap (fresh database).

**Important notes:**

- The admin password in `alfresco.env` is only used when initializing a new repository
- Once the repository is initialized, changing the config file has no effect
- To change the password after initialization, use the Alfresco UI or REST API
- Passwords are stored using bcrypt10 encoding for security (automatic upgrade from MD4 on first login)

**Manually setting admin password:**

If you want to set a specific admin password before installation:

```bash
# Generate MD4 hash for your password (OpenSSL 3.x with legacy provider)
printf '%s' "yourpassword" | iconv -t utf-16le | openssl md4 -provider legacy -provider default

# For OpenSSL 1.x (legacy systems)
printf '%s' "yourpassword" | iconv -t utf-16le | openssl md4
```

Then update `config/alfresco.env`:

```bash
export ALFRESCO_ADMIN_PASSWORD="yourpassword"
export ALFRESCO_ADMIN_PASSWORD_HASH="<hash-from-above>"
```
